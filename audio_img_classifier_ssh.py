...
def send_telegram_alert(host_ip):
    ssh_command=subprocess.run(f"ssh {host_ip} /Users/priscilla/anaconda3/envs/ssh/bin/python /Users/priscilla/Developer/FYP/SSH.py", shell=True, capture_output=True)
    print(ssh_command.stdout.decode())
    if ssh_command.returncode == 0:
        ssh_exit_command=subprocess.run(f"ssh {host_ip} exit", shell=True, capture_output=True)
        print(ssh_exit_command.stdout.decode())
